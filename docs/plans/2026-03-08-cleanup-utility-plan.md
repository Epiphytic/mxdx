...
Expected: `[ 'parseOlderThan', 'cleanupDevices' ]`
  ...
  for (let page = 0; page < 10; page++) {
    ...
    for (const chunk of (data.chunk || [])) {
      ...
      if (t.startsWith('org.mxdx.') || t === 'm.room.encrypted') {
        // If olderThan set, only include events older than cutoff
...
The cleanup command should: parse comma-separated targets, validate them, parse `--older-than` via `parseOlderThan()` from `@mxdx/core`, connect, get session credentials, pass `olderThan` cutoff to each cleanup function, run preview for each target, display results, prompt for confirmation (unless `--force-cleanup`), then execute.

Options: `--force-cleanup` (skip confirmation), `--older-than <duration>` (e.g. `2w`, `1m`, `7d`).
...
Key: `{ isDefault: true }` on the `start` command ensures `mxdx-launcher` (no subcommand) still starts the agent. The `cleanup` command accepts `--older-than <duration>` and `--force-cleanup` options, identical to the client.
...
- "Older Than" text input (optional, placeholder: e.g. `2w`, `1m`, `7d`)
...
Import cleanup functions and `parseOlderThan` via `@mxdx/core/cleanup.js`. Parse the "Older Than" field with `parseOlderThan()` before passing to cleanup functions. Get session credentials from `loadSession()` (IndexedDB).
...
**Step 3: Test with --older-than filter**

```bash
node packages/client/bin/mxdx-client.js cleanup devices --older-than 1d
```

Expected: Only devices with `last_seen_ts` older than 1 day shown in preview.
